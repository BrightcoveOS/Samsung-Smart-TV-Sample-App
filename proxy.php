...
if ( !$url ) {
  ...
  $status = array( 'http_code' => 'ERROR' );
	...
	if (count($_GET) > 1 && strtolower($_SERVER['REQUEST_METHOD']) == 'get') {
		...
		foreach($_GET as $name => $value) {
			$log->logInfo("HERE");
		  if ($name !== "url" ) $parsed['query'] .= "&".urlencode($name).'='.urlencode($value);
		}
		$url = http_build_url(null,$parsed);
	}
  	$ch = curl_init( $url );
 ...
 $log->logInfo("Current Url: ".$status['url']);
...
if ( $_GET['mode'] == 'native' ) {
  ...
  $json = json_encode( $data['contents'],JSON_NUMERIC_CHECK);
  // http_response_code($data['status']['http_code']);
